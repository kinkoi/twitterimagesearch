package com.kaka.app.til.utils.loader;
...
/**
 * Created by satalin on 10/31/14.
 */
public abstract class BaseLoader<T> extends AsyncTaskLoader<T> {
    ...
    public BaseLoader(Context ctx, Uri uri) {
        ...
        super(ctx);
        this.uris = uri;
        logger.warn("init for observer uri: {} ", uris);
    ...
    public T loadInBackground() {
        ...
        T data = null;
        try {
            data = doLoadInBackground();
        } catch (Exception e) {
            logger.warn(e.toString(), e);
        }
        if (mObserver != null) {
            logger.warn("register observer for " + uris);
            if (uris != null) {
                getContext().getContentResolver().registerContentObserver(uris, true, mObserver);
            }
        }
...
// Please don’t hesitate to leave a comment if you still find this confusing! :)
    ...
    class SampleObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            BaseLoader.this.onContentChanged();
        }
    }
}
